...
function update() {
  ...
  for (const e of enms) {
    ...
    if (pl.inv <= 0 && hit(pl.x, pl.y, pl.w, pl.h, e.x, e.y, e.w, e.h)) {
      if (e.spiky) { loseLife(); return; }
      else {
        if (pl.vy > 0 && pl.y + pl.h < e.y + e.h * 0.52) { e.alive = false; pl.vy = -12; score += 50; dieFx(e.x + 15, e.y + 14); sfxStomp(); ui(); }
