...
let checkpoint = { x: 80, active: false, respawnY: null };

function genWorld() {
  plats = []; enms = []; coinArr = []; parts = [];
  checkpoint = { x: 80, active: false, respawnY: null };
...
function genStage1() {
  ...
  const gndRanges = [
    { left: 200,  right: 770  },
    { left: 1090, right: 1960 },
    { left: 2110, right: 3150 },
  ];
  ...
  plats.push({ x: 1600, y: GND() - 60, w: 20, h: 60, t: 'checkpoint', respawnY: GND() });
}
...
function genStage2() {
  ...
  // チェックポイント用の浮き床（index7: x:1430, h:110）
  const cpPlatH = 110;
  const cpPlatY = GND() - cpPlatH;
  ...
  // チェックポイントを浮き床(x:1430,h:110)の上に配置、リスポーンもその浮き床の上
  plats.push({ x: 1465, y: cpPlatY - 60, w: 20, h: 60, t: 'checkpoint', respawnY: cpPlatY - PH });
}
...
function genStage3() {
  ...
  plats.push({ x: 1400, y: GND() - 60, w: 20, h: 60, t: 'checkpoint', respawnY: GND() });
}
